...
app.get('/api/data', async (req, res) => {
  ...
  try {
    const [
      ...
      pool.execute(
        'SELECT id, name, totalMonthlyGoal, avatar FROM users WHERE id = ?',
        [userId]
      ),
      pool.execute(
...
app.post('/api/chat', async (req, res) => {
  ...
  // Build dynamic system prompt from real-time DB data
  let systemPrompt
  try {
  ...
  try {
    ...
    const rawReply = qwenData.choices?.[0]?.message?.content ?? 'Maaf, saya tidak dapat merespons saat ini.'
...
app.delete('/api/subscriptions', async (req, res) => {
  ...
  try {
    const [result] = await pool.execute(
      'DELETE FROM subscriptions WHERE serviceName = ?',
      [serviceName]
    )
...
app.post('/api/user/profile', async (req, res) => {
  const { userId, persona, financial_goals, money_constraints } = req.body
  ...
  try {
    await pool.execute(
      `INSERT INTO user_profiles (user_id, persona_type, financial_goals, money_constraints)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         persona_type      = VALUES(persona_type),
         financial_goals   = VALUES(financial_goals),
         money_constraints = VALUES(money_constraints)`,
      [userId, persona, financial_goals, money_constraints]
    )
    res.json({ success: true })
  } catch (err) {
...
async function buildSystemPrompt() {
  const [[userRows], [budgetRows], [[totalRow]], [subRows]] = await Promise.all([
    pool.execute('SELECT name FROM users LIMIT 1'),
    pool.execute('SELECT categoryName, `limit`, currentSpent FROM budgets'),
    pool.execute(
      `SELECT COALESCE(SUM(amount), 0) AS total
       FROM transactions
       WHERE YEAR(date) = YEAR(CURDATE()) AND MONTH(date) = MONTH(CURDATE())`
    ),
    pool.execute('SELECT serviceName, amount, isVampireRisk, vampireReason FROM subscriptions'),
  ])
...
7. Hanya gunakan SATU token aksi per respons. Token adalah sinyal sistem — jangan disebutkan kepada pengguna.`
}

...
const SOURCE_COLORS = {
  ...
  Default:   { dark_bg: 'rgba(255,255,255,0.07)', dark_text: '#8892B0' },
}
...
function TransactionCard({ tx }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3 transition-all duration-150 active:bg-white/[0.02] rounded-xl px-1">
      {/* Emoji circle */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium"
            style={{ background: src.dark_bg, color: src.dark_text }}>
            {tx.paymentSource}
          </span>
        </div>
      </div>
...
function DateGroup({ group }) {
  return (
    <div>
      ...
      <div className="rounded-2xl bg-card divide-y divide-white/[0.04]" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
        {group.items.map((tx) => (
...
export default function Timeline() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Sticky header */}
      <div className="flex-shrink-0 px-4 pt-4 pb-3 bg-bg border-b border-white/[0.04]">
        ...
        <div className="flex items-center gap-2.5 bg-card rounded-xl px-3.5 py-2.5 mb-3"
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4B5280" strokeWidth="2.5">
          ...
          {FILTERS.map(f => (
            <button key={f}
              ...
              className={`flex-shrink-0 text-xs font-semibold px-3 py-1.5 rounded-full transition-all duration-150 ${
                activeFilter === f
                  ? 'bg-accent text-bg'
                  : 'bg-card2 text-text-muted border border-white/[0.07]'
              }`}>
              {f}
            </button>
          ))}
